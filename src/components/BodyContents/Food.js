...
function Food({ food }) {
  ...
  return (
    <div className={food.isAvailable ? "col-span-2 md:col-span-1 grid grid-cols-5 cursor-pointer bg-[#E6DBB3]/60 p-3 rounded-lg gap-x-2 transition-all duration-500 hover:bg-[#E6DBB3]/80" : "food-unavailable"}>
            ...
            <AnimatePresence>
              {inOrderList ? (
                ...
                <motion.button
                  ...
                  onClick={() => setInOrderList(true)} className='flex justify-end'>
                  <button className='select-none px-2 ml-2 mt-5 text-[#ff9f1c] font-bold py-1 border-b-4 border-[#ff9f1c] transition-all duration-500 hover:text-[#6c757d] hover:border-[#6c757d]'>ثبت سفارش</button>
                </motion.button>
              )}
            </AnimatePresence>
          </>
